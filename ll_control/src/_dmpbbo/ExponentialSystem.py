...
#
u""" Module for the ExponentialSystem class. """
...
class ExponentialSystem(DynamicalSystem):
    u""" A dynamical system representing exponential decay.
    """

    def __init__(self, tau, x_init, x_attr, alpha):
        u""" Initialize an ExponentialSystem.
    ...
    @y_attr.setter
    def y_attr(self, y):
        u""" Set the y part of the attractor state.
    ...
    @property
    def x_attr(self):
        u""" Get the y part of the attractor state.
    ...
    @x_attr.setter
    def x_attr(self, x):
        u""" Set the attractor state.

        Note that for an ExponentialSystem y is equivalent to x.
        """
        if x.size != self._dim_x:
            raise ValueError("y_attr must have size {}".format(self._dim_x))
