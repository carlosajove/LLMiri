...
#
u""" Module for the DynamicalSystem class. """
...
class DynamicalSystem(object):
    ...
    u""" Dynamical systems, which can be numerically integrated or analytically solved.
    """

    def __init__(self, order, tau, y_init, n_dims_x=None):
        u""" Initialize a first or second order dynamical system.
        ...
        if order < 1 or order > 2:
            raise ValueError(u"order should be 1 or 2")
    ...
    @property
    def dim_x(self):
        u""" Get the dimensionality of the state of the dynamical system.
    ...
    @property
    def dim_y(self):
        u""" Get the dimensionality of the y-part of the state of the dynamical system.
    ...
    @property
    def x_init(self):
        u""" Get the initial state of the dynamical system.
        """
        return self._x_init

    @x_init.setter
    def x_init(self, new_x_init):
        u""" Set the initial state of the dynamical system.

        @param new_x_init: Initial state of the dynamical system.
        """
        if new_x_init.size != self._dim_x:
            raise ValueError("x_init must have size {}".format(self._dim_x))
    ...
    def y_init(self, y_init_new):
        u""" Set the y part of the initial state of the dynamical system.

        @param y_init_new: Initial state of the dynamical system.
        """
        if y_init_new.size != self._dim_y:
            raise ValueError("y_init_new must have size {}".format(self._dim_y))
    ...
    def integrate_start(self, y_init=None):
        u""" Start integrating the system with a new initial state.
    ...
    def integrate_step(self, dt, x):
        u""" Integrate the system one time step.
    ...
    def integrate_step_euler(self, dt, x):
        u""" Integrate the system one time step using Euler integration.
        ...
        if x.size != self._dim_x:
            raise ValueError(u"x must have size {self._dim_x}")
    ...
    def integrate(self, ts, **kwargs):
        int_method = kwargs.get(u"integration_method", u"Runge-Kutta")
        ...
        if int_method == u"Runge-Kutta":
            for ii in xrange(1, n_time_steps):
                dt = ts[ii] - ts[ii - 1]
    ...
    def decouple_parameters(self):
        u""" Decouple the parameters of a multidimensional system.
    ...
    def plot(self, ts, xs, xds, **kwargs):
        u"""Plot the output of the integration of a dynamical system.
        ...
        if u"axs" in kwargs and kwargs[u"axs"] is not None:
            axs = kwargs[u"axs"]
        else:
            # 2 => plot x and xd, 3 => plot y, yd and ydd=zd/tau
            n_plots = 2 if dim_x == dim_y else 3
            fig = kwargs.get(u"fig") or plt.figure(figsize=(5 * n_plots, 4))
            axs = [fig.add_subplot(1, n_plots, p + 1) for p in xrange(n_plots)]

        # Prepare tex interpretation
        plt.rc(u"text", usetex=True)
        plt.rc(u"font", family=u"serif")

        if dim_x == dim_y:

            lines = axs[0].plot(ts, xs)
            axs[0].set_ylabel(ur"$x$")

            if len(axs) > 1:
                lines[len(lines) :] = axs[1].plot(ts, xds)
                axs[1].set_ylabel(ur"$\dot{x}$")
            ...
            axs[0].set_ylabel(ur"$y$")

            if len(axs) > 1:
                lines[len(lines) :] = axs[1].plot(ts, yds)
                axs[1].set_ylabel(ur"$\dot{y} = z/\tau$")

            if len(axs) > 2:
                lines[len(lines) :] = axs[2].plot(ts, zds / self._tau)
                axs[2].set_ylabel(ur"$\ddot{y} = \dot{z}/\tau$")
