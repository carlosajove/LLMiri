...
u""" Module for the FunctionApproximator class. """
...
class FunctionApproximator(Parameterizable):
    u"""Base class for all function approximators.
    ...
    def __init__(self, meta_params, model_param_names):
        ...
        self._model_params = dict((name, None) for name in model_param_names)
    ...
    def train(self, inputs, targets, **kwargs):
        u""" Train the function approximator with data.
    ...
    def predict(self, inputs):
        u""" Make predictions for (new) input data.

        @param inputs: Input data (n_samples X n_dims_input)
        @return: Predictions (n_samples X n_dims_output)
        """
        if not self.is_trained():
            raise ValueError(u"Calling predict() on untrained function approx.")

        # Ensure n_dims=2, i.e. shape = (30,) => (30,1)
        inputs = inputs.reshape(inputs.shape[0], -1)
        if inputs.shape[1] != self._dim_input:
            raise ValueError(
                u"Dimensionality of inputs for predict() must be the same as for train()."
            )

        return self._predict(inputs, self._model_params)

    def dim_input(self):
        u""" Return the dimensionality of the inputs of the function_approximator
        @return: The dimensionality of the inputs of the function_approximator.
        """
        return self._dim_input

    def is_trained(self):
        u"""Determine whether the function approximator has already been trained with data or not.
    ...
    def _train(inputs, targets, meta_params, **kwargs):
        u"""Train the function approximator with input and target examples.
    ...
    def _predict(inputs, model_params):
        u"""Query the function approximator to make a prediction.
    ...
    def get_param_vector(self):
        ...
        if not self.is_trained():
            raise ValueError(u"FunctionApproximator is not trained.")
    ...
    def set_param_vector(self, values):
        ...
        if len(values) != self.get_param_vector_size():
            raise ValueError(
                "values ({}) should have same size as size of selected parameter vector".format(len(values)),
                "({}) ".format(self.get_param_vector_size())
            )
    ...
    def _get_axis(self, fig=None):
        ...
        if self.dim_input() == 1:
            ...
            raise ValueError("Cannot create axis with dim_input = {}".format(self.dim_input()))
    ...
    def _get_grid(inputs_min, inputs_max, n_samples_per_dim=None):
        ...
        if n_dims == 1:
            ...
            raise ValueError("Cannot create axis with n_dims = {}.".format(n_dims))

        return inputs_grid, n_samples_per_dim

    @staticmethod
    def _plot_grid_values(inputs, activations, ax, n_samples_per_dim):
        u"""Plot basis functions activations, whilst being smart about the dimensionality of input
        data. """

        n_dims = len(np.atleast_1d(inputs[0]))
        if n_dims == 1:
            lines = ax.plot(inputs, activations, u"-")

        elif n_dims == 2:
            n_basis_functions = len(activations[0])
            basis_functions_to_plot = xrange(n_basis_functions)
            ...
            print "Cannot plot input data with a dimensionality of {}".format(n_dims)
            lines = []
    ...
    def plot_model_parameters(self, inputs_min, inputs_max, **kwargs):
        u""" Plot a representation of the model parameters on a grid.
    ...
    def plot_predictions_grid(self, inputs_min, inputs_max, **kwargs):
        u""" Plot the predictions of the function approximator on a grid of input values.

        @param inputs_min: The min values for the grid
        @param inputs_max:  The max values for the grid
        @return: line handles and axis
        """
        ax = kwargs.get(u"ax") or self._get_axis()
        ...
        if self.dim_input() == 1:
            ...
            print("Cannot plot input data with a dimensionality of {}".format(self.dim_input()))
    ...
    def plot_predictions(self, inputs, **kwargs):
        u""" Plot the predictions of a function approximator for given inputs.

        @param inputs: The input samples (n_samples X n_input_dims )
        @return: line handles and axis
        """
        targets = kwargs.get(u"targets", [])
        ax = kwargs.get(u"ax") or self._get_axis()
        plot_residuals = kwargs.get(u"plot_residuals", True)

        outputs = self.predict(inputs)

        # If only few data points, plot individual markers, otherwise plot lines
        line_style = u"o" if inputs.shape[0] < 40 else u"-"
        ...
        if self.dim_input() == 1:
            ...
            raise ValueError("Cannot plot input data with dim_input() = {}".format(self.dim_input()))
    ...
    def plot(self, inputs=None, **kwargs):
        ...
        ax = kwargs.get(u"ax") or self._get_axis()
        targets = kwargs.get(u"targets", self._targets_training)
        plot_residuals = kwargs.get(u"plot_residuals", True)
        plot_model_parameters = kwargs.get(u"plot_model_parameters", False)
