...
#
u""" Module for the SpringDamperSystem class. """
...
class SpringDamperSystem(DynamicalSystem):
    u""" A dynamical system representing a spring-damper system.
    """
    ...
    def __init__(
        ...
        y_attr,
        damping_coefficient,
        spring_constant=u"CRITICALLY_DAMPED",
        mass=1.0,
    ):
        u""" Initialize a SpringDamperSystem.
        ...
        @param mass: Mass
        """
        super(SpringDamperSystem, self).__init__(2, tau, y_init)
        self._y_attr = y_attr
        self.damping_coefficient = damping_coefficient
        self.mass = mass
        if spring_constant == u"CRITICALLY_DAMPED":
    ...
    def y_attr(self, new_y_attr):
        u""" Set the y part of the attractor state, where x = [y z]
        """
        if new_y_attr.size != self._dim_y:
            raise ValueError("y_attr must have size {}".format(self._dim_y))
    ...
    def analytical_solution(self, ts):
        ...
        for i_dim, zeta in enumerate(zetas):
            if zeta != 1.0:
                print ("WARNING: Spring-damper system is not critically damped for dim={} zeta".format(i_dim), 
                      "={}".format(zeta))

        for i_dim in xrange(self._dim_y):
