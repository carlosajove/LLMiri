...
from .DynamicalSystem import DynamicalSystem      #TODO
from .ExponentialSystem import ExponentialSystem  #TODO
from .RichardsSystem import RichardsSystem        #TODO
from .SigmoidSystem import SigmoidSystem          #TODO
from .SpringDamperSystem import SpringDamperSystem  #TODO
from .TimeSystem import TimeSystem                 #TODO
from .Parameterizable import Parameterizable  #TODO
...
class Dmp(DynamicalSystem, Parameterizable):
    u""" Dynamical Movement Primitive
    ...
    def __init__(self, tau, y_init, y_attr, function_approximators, **kwargs):
        u"""Initialize a DMP with function approximators and subsystems
        ...
        super(Dmp, self).__init__(1, tau, y_init, dim_dmp)

        self._y_attr = y_attr
        self._function_approximators = function_approximators

        transformation_system = kwargs.get(u"transformation_system", None)
        if transformation_system:
            self._spring_system = transformation_system
        else:
            alpha = kwargs.get(u"alpha_spring_damper", 20.0)
        ...
        dmp_type = kwargs.get(u"dmp_type", u"KULVICIUS_2012_JOINING")
        if dmp_type in [u"IJSPEERT_2002_MOVEMENT", u"Ijs02"]:
        ...
        elif dmp_type in [u"KULVICIUS_2012_JOINING", u"Kul12", u"COUNTDOWN_2013"]:
            ...
            count_down = dmp_type == u"COUNTDOWN_2013"
            phase_system_default = TimeSystem(tau, count_down)

        elif u"2022" in dmp_type or u"SCT23" in dmp_type:
            self.goal_system_requires_scaling = u"NO_SCALING" not in dmp_type
            ...
            raise ValueError("Unknown dmp_type: {}".format(dmp_type))

        # Check if subsystems are specified in kwargs. If not, use default.
        self._phase_system = kwargs.get(u"phase_system", phase_system_default)
        self._gating_system = kwargs.get(u"gating_system", gating_system_default)
        self._goal_system = kwargs.get(u"goal_system", goal_system_default)
        self._damping_system = kwargs.get(u"damping_system", damping_system_default)

        # Initialize variables related to scaling of the forcing term
        self._forcing_term_scaling = kwargs.get(u"forcing_term_scaling", u"NO_SCALING")
        self._scaling_amplitudes = kwargs.get(u"scaling_amplitudes", None)
    ...
    def from_traj(cls, trajectory, function_approximators, **kwargs):
        u"""Initialize a DMP by training it from a trajectory.
    ...
    def dim_dmp(self):
        u""" Get the dimensionality of the dmp (y-part)
    ...
    def integrate_start(self, y_init=None):
        u""" Start integrating the DMP with a new initial state.
    ...
    def differential_equation(self, x):
        u"""The differential equation which defines the system.
        ...
        if self._forcing_term_scaling == u"G_MINUS_Y0_SCALING":
            g_minus_y0 = self._y_attr - self._y_init
            forcing_term = forcing_term * g_minus_y0

        elif self._forcing_term_scaling == u"AMPLITUDE_SCALING":
            if self._scaling_amplitudes is None:
                raise ValueError(u"Cannot do AMPLITUDE_SCALING without scaling amplitudes.")
    ...
    def _compute_func_approx_predictions(self, phase_state):
        u"""Compute the outputs of the function approximators.
        ...
        if self._function_approximators:
            print("HEY")
            for i_fa in xrange(self.dim_dmp()):
                print("ehehe")
    ...
    def analytical_solution(self, ts=None, suppress_forcing_term=False):
        u"""Return analytical solution of the system at certain times
        ...
        if ts is None:
            if self._ts_train is None:
                raise ValueError(
                    u"Neither the argument 'ts' nor the member variable self._ts_train was set."
                )
            else:
        ...
        for tt in xrange(1, n_time_steps):
            dt = ts[tt] - ts[tt - 1]
    ...
    def train(self, trajectory, **kwargs):
        u"""Train a DMP with a trajectory.
        ...
        if u"function_approximators" in kwargs:
            self._function_approximators = kwargs.get(u"function_approximators")
        ...
        if kwargs.get(u"save_training_data", False):
    ...
    def _compute_targets(self, trajectory):
        u"""Given a trajectory, compute the inputs and targets for the function approximators.
        ...
        if self.dim_dmp() != dim_data:
            raise ValueError(u"dims of trajectory data and dmp must be the same")
    ...
    def states_as_pos_vel_acc(self, x_in, xd_in):
        u""" Convert the dynamical system states into positions, velocities and accelerations.
    ...
    def states_as_trajectory(self, ts, x_in, xd_in):
        u"""Get the output of a DMP dynamical system as a trajectory.
    ...
    def y_init(self, y_init_new):
        u""" Set the initial state (y-part)

        @param y_init_new: New initial state (y-part)
        """
        if y_init_new.size != self.dim_dmp():
            raise ValueError(u"y_init must have same size {self.dim_dmp()}")
    ...
    @property
    def y_attr(self):
        u""" Return the y part of the attractor state, where x = [y z]
        """
        return self._y_attr
    ...
    @property
    def ts_train(self):
        u""" Get the time steps of the trajectory on which the DMP was trained.
    ...
    def set_selected_param_names(self, names):
        u""" Get the names of the parameters that have been selected.
    ...
    def set_params_dyn_sys(self, params):
        for system_name in [u"spring_system", u"goal_system", u"damping_system"]:
            attr_name = u"_" + system_name
            if system_name in params:
                system = self.__dict__[attr_name]
                if not system:
                    raise Exception('Trying to set param of {}, but it is None'.format(system_name))
                ...
                for param_name in params[system_name]:
                    if param_name in system.__dict__:
                        ...
                        raise Exception(
                            'Unknown param {} in {} of {}'.format(param_name, system_name, system.__class__)
                        )

        # To check and give feedback
        for system_name in params:
            if u"_" + system_name not in self.__dict__:
                raise Exception('Unknown system {}'.format(system_name))
    ...
    def get_param_vector(self):
        ...
        if u"goal" in self._selected_param_names:
    ...
    def set_param_vector(self, values):
        ...
        if len(values) != size:
            raise ValueError("values must have size {}".format(size))
        offset = 0
        if self._function_approximators is not None:
            for fa in self._function_approximators:
                if fa.is_trained():
    ...
    def get_dmp_axes(n_cols=4, **kwargs):
        u""" Get matplotlib axes on which to plot the output of a DMP.

        @param n_cols: Number of columns for the subplots.
        @return: list of axes on which the DMP was plotted
        """
        plot_compact = kwargs.get(u"plot_compact", False)
        ...
        axs = [fig.add_subplot(n_rows, n_cols, i + 1) for i in xrange(n_cols)]
        if not plot_compact:
            axs.extend([fig.add_subplot(n_rows, n_cols, i + n_cols + 1) for i in xrange(n_cols)])
        return axs

    def plot(self, ts=None, xs=None, xds=None, **kwargs):
        u""" Plot the output of the DMP.
        ...
        if xs is None:
            ...
            forcing_terms = kwargs.get(u"forcing_terms", [])
            fa_outputs = kwargs.get(u"fa_outputs", [])

        n_subplot_columns = kwargs.get(u"n_subplot_columns", 4)
        axs = kwargs.get(u"axs") or Dmp.get_dmp_axes(n_subplot_columns, **kwargs)

        plot_tau = kwargs.get(u"plot_tau", True)
        # has_fa_output = len(forcing_terms) > 0 or len(fa_outputs) > 0
        plot_no_forcing_term_also = kwargs.get(u"plot_no_forcing_term_also", False)
        plot_demonstration = kwargs.get(u"plot_demonstration", False)

        d = self.dim_dmp()  # noqa Abbreviation for convenience
        plot_compact = kwargs.get(u"plot_compact", False)
        if not plot_compact:
            systems = [
                (u"goal", self.GOAL, axs[0:1], self._goal_system),
                (u"spring-damper", self.SPRING, axs[1:4], self._spring_system),
                (u"phase", self.PHASE, axs[4:5], self._phase_system),
                (u"gating", self.GATING, axs[6:7], self._gating_system),
            ]
        else:
            systems = [
                (u"spring-damper", self.SPRING, axs[0:3], self._spring_system),
                (u"goal", self.GOAL, axs[0:1], self._goal_system),
                (u"gating", self.GATING, axs[4:5], self._gating_system),
            ]
        ...
        for system in systems:
            ...
            if system[3]:
                ...
                if plot_demonstration and system[0] == u"spring-damper":
                    h_demo, _ = plot_demonstration.plot(axs_cur)
                    plt.setp(h_demo, linestyle=u"-", linewidth=4, color=(0.6, 0.6, 0.6), alpha=0.5)

                if plot_no_forcing_term_also and system[0] == u"spring-damper":
                    ...
                    plt.setp(h_no, linestyle=u"--", linewidth=1)  # Make smaller and dashed
            ...
            if not plot_compact:
                for i, ax in enumerate(axs_cur):
                    ...
                    ax.text(x, y, system[0], horizontalalignment=u"center")
        ...
        if len(forcing_terms) > 1:
            ...
            y = np.mean(ax.get_ylim())
            if not plot_compact:
                ax.text(x, y, u"forcing term", horizontalalignment=u"center")
            ax.set_xlabel(ur"time ($s$)")
            ax.set_ylabel(ur"$v\cdot f_{\mathbf{\theta}}(x)$")
        ...
        if len(fa_outputs) > 1:
            ...
            y = np.mean(ax.get_ylim())
            if not plot_compact:
                ax.text(x, y, u"func. approx.", horizontalalignment=u"center")
            ax.set_xlabel(ur"time ($s$)")
            ax.set_ylabel(ur"$f_\mathbf{\theta}(x)$")

        if plot_tau:
            for ax in axs:
                ax.axvline(self._tau, color=u"k", linewidth=1)
    ...
    def plot_comparison(self, trajectory, **kwargs):
        ts = kwargs.get(u"ts", trajectory.ts)
        xs, xds, _, _ = self.analytical_solution(ts)
        traj_reproduced = self.states_as_trajectory(ts, xs, xds)

        axs = kwargs.get(u"axs", None)
        h_demo, axs = trajectory.plot(axs)
        h_repr, axs = traj_reproduced.plot(axs)

        plt.setp(h_demo, linestyle=u"-", linewidth=4, color=(0.8, 0.8, 0.8))
        plt.setp(h_demo, label=u"demonstration")
        plt.setp(h_repr, linestyle=u"--", linewidth=2, color=(0.0, 0.0, 0.5))
        plt.setp(h_repr, label=u"reproduced")
